use super::{ConceptId, ConceptKind, ConceptMap, Pangine, ProjectionAssignment, QuestionCandidateWitnesses, QuestionExperience};
use crate::Relevance;
...
pub enum CompletionRemainderSide {
    /// The selected experience contained structure not consumed by the question.
    Experience,
    /// The question contained context not supplied by the selected experience.
    Question,
}
...
/// Describes one selected experience fragment participating in a completion.
#[derive(Clone, PartialEq, Eq, PartialOrd, Ord)]
...
impl CompletionEvidence {
    ...
    /// Returns the selected Percept owning the exact source root.
    pub fn source_percept(&self) -> &ConceptId {
        &self.experience.source.percept
    }

    /// Returns the exact experienced root supplying this evidence.
    pub fn source_root(&self) -> &ConceptId {
        &self.experience.source.root
    }

    /// Returns the occurrence count of the exact source root.
    pub fn source_occurrences(&self) -> u64 {
        self.experience.source.occurrences
    }

    /// Returns the recursive root view matched by the clause.
    pub fn matched(&self) -> &ConceptId {
        &self.experience.matched
    }
...
impl Completion {
    ...
    /// Returns the experience fragments participating in this completion.
...
impl Pangine {
    /// Completes a structural question against exact roots selected by Percept.
    ///
    ...
    pub(super) fn complete_question_snapshot(&mut self, question: &ConceptId, snapshot: &super::QuestionSnapshot) -> CompletionResult {
        ...
        for clause in clauses {
            ...
            for experience in &snapshot.experiences {
                for completion in self.structural_completions(&experience.matched, &clause, &mut Vec::new()) {
    ...
    pub(super) fn materialize_completion_bindings(&mut self, result: &CompletionResult) -> BTreeMap<ConceptId, Option<ConceptId>> {
        let mut outputs = BTreeSet::new();
        ...
        for completion in &result.completions {
            for evidence in &completion.evidence {
                for output in &outputs {
                    ...
                    witnesses.entry(output.clone()).or_default().entry(candidate.clone()).or_default().insert(evidence.experience.source.clone());
                }
            }
        }
    ...
    fn structural_completions(&mut self, experience: &ConceptId, question: &ConceptId, ordered_path: &mut Vec<usize>) -> BTreeSet<StructuralCompletion> {
        if self.is_percept(question) {
            return BTreeSet::from([StructuralCompletion {
                assignment: ProjectionAssignment::from([(question.clone(), experience.clone())]),
        ...
        match (&experience.0.kind, &question.0.kind) {
            (ConceptKind::Named(experience_name), ConceptKind::Named(question_name)) => {
            ...
            (ConceptKind::Ordered { components: experience_components }, ConceptKind::Ordered { components: question_components }) => {
                if experience_components.len() != question_components.len() {
                    return BTreeSet::new();
                }

                let pairs = experience_components
                    .iter()
                    .zip(question_components)
                    .map(|(experience, question)| (experience.clone(), question.clone()))
                    .collect::<Vec<_>>();
                let mut products = exact_structural_completion();
                for (position, (experience_component, question_component)) in pairs.into_iter().enumerate() {
                    ordered_path.push(position);
                    let component = self.structural_completions(&experience_component, &question_component, ordered_path);
                    ordered_path.pop();
            ...
            (ConceptKind::Unordered, ConceptKind::Unordered) => self.unordered_completions(experience, question, ordered_path),
            _ => BTreeSet::new(),
        }
    }

    fn unordered_completions(&mut self, experience: &ConceptId, question: &ConceptId, ordered_path: &[usize]) -> BTreeSet<StructuralCompletion> {
        if experience.0.subconcepts.values().any(|relevance| *relevance != Relevance::DEFAULT)
            || question.0.subconcepts.values().any(|relevance| *relevance != Relevance::DEFAULT)
        {
            return BTreeSet::new();
        }

        let experience_members = experience.0.subconcepts.keys().cloned().collect::<Vec<_>>();
        ...
        if question_fixed.len() <= experience_members.len() {
            for injection in self.inject_members(&question_fixed, &experience_members, true, ordered_path) {
                let remaining = unused_members(&experience_members, &injection.used_candidates);
                if question_outputs.is_empty() {
                    let mut completion = injection.completion;
                    self.add_completion_remainder(&mut completion, CompletionRemainderSide::Experience, ordered_path, &remaining);
        ...
        if question_outputs.is_empty() && experience_members.len() <= question_fixed.len() {
            for injection in self.inject_members(&experience_members, &question_fixed, false, ordered_path) {
