...
fn semicolon_separated_input_returns_the_last_statement() {
    ...
    assert_eq!(must_ref(&mut pangine, "[A][B];[C][D]"), must_ref(&mut pangine, "[C][D]"));
    assert_eq!(must_ref(&mut pangine, "['A'] = [A][B];[C][D];$['A']"), must_ref(&mut pangine, "[A][B]"));
    assert_eq!(pangine.reference_concept("['A'] = [];[C][D];$['A']").unwrap(), None);
}
...
fn c_style_and_cpp_style_comments_are_ignored() {
    ...
    assert_eq!(must_ref(&mut pangine, "[A] /* comment */ [B]"), must_ref(&mut pangine, "[A][B]"));
    assert_eq!(must_ref(&mut pangine, "[A] /* comment \n */ [B]"), must_ref(&mut pangine, "[A][B]"));
    assert_eq!(must_ref(&mut pangine, "[A] // comment [B]"), must_ref(&mut pangine, "[A]"));
    assert_eq!(must_ref(&mut pangine, "[A] // comment \n[B]"), must_ref(&mut pangine, "[A][B]"));
}
...
fn multiline_whitespace_remains_union_in_direct_concept_parsing() {
    ...
    assert_eq!(must_ref(&mut pangine, "[A]\n[B]"), must_ref(&mut pangine, "[A][B]"));
}
...
fn direct_semantics_are_right_associative_and_share_legacy_correlation_identity() {
    ...
    assert_eq!(pangine.get_correlation_a(&semantic), Some(must_ref(&mut pangine, "[A]")));
    assert_eq!(pangine.get_correlation_b(&semantic), Some(must_ref(&mut pangine, "[B]")));
    ...
    assert_eq!(pangine.reference_concept(&grouped_semantic_formatted).unwrap(), Some(grouped_semantic));
}
...
fn canonical_formatting_orders_relevance_shape_and_name_collisions() {
    ...
    assert_eq!(pangine.reference_concept(&relevance_formatted).unwrap(), Some(relevance_order));
    ...
    assert_eq!(pangine.reference_concept(&fewer_formatted).unwrap(), Some(fewer_subconcepts));
    ...
    assert_eq!(pangine.reference_concept(&name_collision_formatted).unwrap(), Some(name_collision));

    let relation_kinds = must_ref(&mut pangine, "(?[a]:[b])*({[a]->[b]})");
    let relation_kinds_formatted = pangine.format_concept(&relation_kinds, false);
    assert_eq!(relation_kinds_formatted, "((?[a]:[b]){[a]->[b]})");
    assert_eq!(pangine.reference_concept(&relation_kinds_formatted).unwrap(), Some(relation_kinds));
}
...
fn script_text_accepts_semicolons_and_legacy_line_statements() {
    ...
    assert_eq!(semicolon_null_script.parse_script_text("[A];[];").unwrap(), None);
}
...
fn binary_inverse_merge_inverts_rhs_merge_operands() {
    ...
    assert_eq!(must_ref(&mut pangine, "([A]/[B])"), must_ref(&mut pangine, "[A]*![B]"));
    assert_eq!(must_ref(&mut pangine, "![A]/[B]"), must_ref(&mut pangine, "![A]*![B]"));
    assert_eq!(must_ref(&mut pangine, "[C]/[B]/[A]"), must_ref(&mut pangine, "[C]*![B]*![A]"));
    assert_eq!(must_ref(&mut pangine, "([A][B])/([B][C])"), must_ref(&mut pangine, "[A]*![C]"));
...
fn explicit_percept_mutation_operators_are_required() {
    ...
    assert!(matches!(pangine.reference_concept("['A'] + [A]"), Err(ParseError::InvalidSyntax)));
    assert!(matches!(pangine.reference_concept("['A'] ~ [A]"), Err(ParseError::InvalidSyntax)));

    assert_eq!(must_ref(&mut pangine, "['A'] += [A]"), must_ref(&mut pangine, "[A]"));
    assert_eq!(must_ref(&mut pangine, "['A'] += [B]"), must_ref(&mut pangine, "[A][B]"));
    assert_eq!(must_ref(&mut pangine, "['A'] -= [A]"), must_ref(&mut pangine, "[B]"));
    assert_eq!(pangine.reference_concept("['A'] -= [B]").unwrap(), None);

    let mut experience = Pangine::new();
    assert_eq!(must_ref(&mut experience, "['A'] ~= [A]"), must_ref(&mut experience, "[A]"));
    assert_eq!(must_ref(&mut experience, "['A'] ~= [B]"), must_ref(&mut experience, "[A][B]"));
    assert_eq!(must_ref(&mut experience, "['A'] ~= ![A]"), must_ref(&mut experience, "[B]"));
...
fn percept_addition_preserves_groups_and_merge_flattens() {
    ...
    assert_eq!(must_ref(&mut pangine, "['A'] += [A][B]"), must_ref(&mut pangine, "[A][B]"));
    assert_eq!(must_ref(&mut pangine, "['M'] *= [A][B]"), must_ref(&mut pangine, "[A][B]"));
    ...
    assert_eq!(must_ref(&mut pangine, "['A'] -= [B][C]"), must_ref(&mut pangine, "[A][B]"));
    assert_eq!(must_ref(&mut pangine, "['M'] /= [B][C]"), must_ref(&mut pangine, "[A][B]"));
