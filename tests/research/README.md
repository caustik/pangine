...
`unified_concept_state.rs` exercises current API behavior around direct root boundaries, repeated observations, finite induction crossover, and generic-match swamping. The corresponding test-only internal prototype lives under `src/engine/research/` because it needs access to matcher internals.
